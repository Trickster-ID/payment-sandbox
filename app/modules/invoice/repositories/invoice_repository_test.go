...
func TestInvoiceRepository_MerchantIDByUserID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		mock.ExpectQuery(regexp.QuoteMeta("SELECT id::text, user_id::text, balance, created_at, updated_at FROM merchants")).
			WithArgs("user-1").
	...
	t.Run("success", func(t *testing.T) {
		mock.ExpectQuery(regexp.QuoteMeta("INSERT INTO invoices")).
			...
			WillReturnRows(sqlmock.NewRows([]string{"id", "merchant_id", "invoice_number", "customer_name", "customer_email", "amount", "description", "due_date", "status", "payment_link_token", "created_at", "updated_at"}).
				AddRow("inv-1", "merchant-1", "INV-123", "Alice", "alice@example.com", int64(1000), "desc", dueDate, "PENDING", "token", now, now))

		inv, err := repo.CreateInvoice("merchant-1", "Alice", "alice@example.com", int64(1000), "desc", dueDate)
		require.NoError(t, err)
...
func TestInvoiceRepository_ListInvoices(t *testing.T) {
	...
	t.Run("success with status", func(t *testing.T) {
		...
		mock.ExpectQuery(regexp.QuoteMeta("SELECT id::text, merchant_id::text, invoice_number, customer_name, customer_email, amount, COALESCE(description, ''), due_date, status::text, payment_link_token, created_at, updated_at FROM invoices WHERE merchant_id=$1 AND deleted_at IS NULL AND status=$2 ORDER BY created_at DESC LIMIT $3 OFFSET $4")).
...
func TestInvoiceRepository_MerchantInvoiceByID(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		mock.ExpectExec(regexp.QuoteMeta("UPDATE invoices")).WillReturnResult(sqlmock.NewResult(0, 0))
		mock.ExpectQuery(regexp.QuoteMeta("SELECT id::text, merchant_id::text, invoice_number, customer_name, customer_email, amount, COALESCE(description, ''), due_date, status::text, payment_link_token, created_at, updated_at FROM invoices")).
